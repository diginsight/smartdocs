...
Each page here has an identifier-bearing companion in the private peer repository `diginsight/smartdocs.internal`, at **the same repository-relative path** — `src/docs/90.00-issues/202608/20260817.01-deployfail/` — with an `.internal.md` suffix. Path parity is the rule, not a coincidence: a companion is located by suffixing the name, never by consulting an index. The companion carries what the public page deliberately omits, including the as-built topology diagrams with real resource names.

| Public page | Internal companion | What the companion adds |
|---|---|---|
| `01-overview.md` | `01-overview.internal.md` | Subscription and resource groups, de-aliased timeline, recovery commands |
| `02-incident-a-deployment-chain.md` | `02-incident-a-deployment-chain.internal.md` | OIDC subjects, credential state before and after, the `az` commands as executed |
| `03-incident-b-docs-site-500.md` | `03-incident-b-docs-site-500.internal.md` | Content store state, the HTTP 409 detail, validation URLs |
| `04-environment-reference.md` | `04-environment-reference.internal.md` | As-built diagrams with real names, address spaces and private endpoint targets |

Alias resolution lives in `src/docs/_aliases/testmc.aliases.internal.md`. These are code-formatted paths rather than links, because the targets do not exist in this repository.

## 🗂️ The two incidents

| | [Incident A](02-incident-a-deployment-chain.md) | [Incident B](03-incident-b-docs-site-500.md) |
|---|---|---|
...
One material gap remains. The network fix lives only in the running environment, so the incident is closed while its cause is not. Read [incident A](02-incident-a-deployment-chain.md) for the CI and identity analysis, [incident B](03-incident-b-docs-site-500.md) for the networking analysis, and [the environment reference](04-environment-reference.md) for the topology and constraints that apply to the next deployment.
