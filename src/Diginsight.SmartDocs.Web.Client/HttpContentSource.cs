...
/// <summary>
/// Client-side content source: fetches raw Markdown from the server's <c>/_content-raw/{key}</c>
...
public sealed class HttpContentSource(HttpClient http) : IContentSource
{
    ...
    {
        using HttpResponseMessage response = await http.GetAsync($"_content-raw/{contentKey}", ct);
