...
description: "When and how to build multi-agent orchestrations — architecture tiers, 9 design principles, subagent mechanics, four-specialist pattern"
domain: "prompt-engineering"
authoritative_sources:
  - url: "https://code.visualstudio.com/docs/agent-customization/custom-agents"
    description: "VS Code custom agents — multi-agent orchestration, runSubagent, subagent mechanics"
goal: "Provide decision framework and design principles for determining when and how to build multi-agent orchestrations"
scope:
  covers:
    - "Architecture tier decision (single prompt → full orchestrator)"
    - "9 design principles (start simple, define by tools, explicit contracts, iteration limits)"
    - "Subagent mechanics and when to use"
    - "Four-specialist pattern composition"
    - "Context isolation vs. complexity trade-offs"
  excludes:
    - "Handoff contracts (see 02.01-handoffs-pattern.md)"
    - "Runtime validation (see 04.04-orchestrator-runtime-validation.md)"
boundaries:
  - "MUST start with the tier decision tree before specifying architecture"
  - "MUST NOT use orchestration as default — justify added complexity"
rationales:
  - "Architecture tier decision prevents over-engineering simple tasks"
  - "Design principles ensure orchestrations remain maintainable as complexity grows"
---
...
- **Internal**: [03.03-agent-hooks-reference.md](./03.03-agent-hooks-reference.md), [02.02-context-window-and-token-optimization.md](./02.02-context-window-and-token-optimization.md)
- **Sources** (practical examples):
  - [10.00-how_to_design_orchestrator_prompts.md](https://learn-testmc-app-itn-01.azurewebsites.net/03.00-tech/05.02-prompt-engineering/04-howto/10.00-how-to-design-orchestrator-prompts) — four-specialist pattern, phase-based coordination (linear, iterative, parallel), use case challenge 5-step methodology, execution flow control (iteration limits, recursion prevention, error handling)
  - [11.00-how_to_design_subagent_orchestrations.md](https://learn-testmc-app-itn-01.azurewebsites.net/03.00-tech/05.02-prompt-engineering/04-howto/11.00-how-to-design-subagent-orchestrations) — subagent mechanics, runSubagent tool patterns, context isolation examples
- **External**: [VS Code Multi-Agent](https://code.visualstudio.com/docs/agent-customization/custom-agents)
