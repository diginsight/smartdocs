...
description: "8 lifecycle events, JSON configuration schema, and I/O protocol for deterministic automation via agent hooks"
domain: "prompt-engineering"
authoritative_sources:
  - url: "https://code.visualstudio.com/docs/agent-customization/hooks"
    description: "VS Code agent hooks — lifecycle events (onSave, onStart, onEnd), JSON schema, I/O protocol"
goal: "Provide quick-reference documentation for agent hook lifecycle events and configuration schema"
scope:
  covers:
    - "8 lifecycle events (SessionStart, UserPromptSubmit, PreToolUse, PostToolUse, SubagentStart, SubagentStop, PreCompact, etc.)"
    - "When to use hooks vs. other mechanisms"
    - "JSON configuration schema and field descriptions"
    - "I/O protocol (stdin/stdout communication)"
    - "Event ordering and deterministic guarantees"
  excludes:
    - "Hook implementation examples (see hook creation workflows)"
    - "General agent architecture (see 02.03-orchestrator-design-patterns.md)"
boundaries:
  - "MUST provide deterministic lifecycle automation, not LLM reasoning"
  - "MUST NOT use hooks for complex decision logic"
rationales:
  - "Hooks provide guaranteed execution outside agentic loop variability"
  - "Clear event taxonomy prevents race conditions and ordering bugs"
---
...
- **Internal**: [02.03-orchestrator-design-patterns.md](./02.03-orchestrator-design-patterns.md)
- **Source** (practical examples):
  - [09.00-how_to_use_agent_hooks_for_lifecycle_automation.md](https://learn-testmc-app-itn-01.azurewebsites.net/03.00-tech/05.02-prompt-engineering/04-howto/09.00-how-to-use-agent-hooks-for-lifecycle-automation) — JSON configuration format, 8 lifecycle events with schemas, hook I/O protocol (event-specific input/output), 6 advanced patterns (block commands, auto-format, inject context, log usage, approvals, test enforcement), common pitfalls
- **External**: [VS Code Agent Hooks](https://code.visualstudio.com/docs/agent-customization/hooks)
...
| 1.2.0 | 2026-06-01 | Added Observability section noting OpenTelemetry signals (VS Code 1.122+) | System |
