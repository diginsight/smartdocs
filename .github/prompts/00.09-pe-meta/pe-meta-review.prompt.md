...
Before emitting the Phase 8 report, the orchestrator MUST self-check the coverage-evidence markers against the outcome log AND the resolved invocation. This is the runtime guard for the [2026-06-06 full-processing issue](../../../src/docs/90.00-issues/202606/20260606.02-pe-meta-update/01-small-instruction-files-changes/overview.md), where a `breadth=full` + `--mode apply` run collapsed into a frontmatter-only metadata scan (1 of ~35 dimensions, no Phase 1 research, no per-artifact review) yet still emitted a structurally valid success report. It mirrors the plan-file linter pattern above: a load-bearing `MUST` paired with a hard runtime invariant.
