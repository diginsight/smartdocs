...
- A published page MAY cite the **path** of its own internal companion as a signpost, stating what the companion adds. That pointer MUST be a backticked path rather than a Markdown link, because the target does not resolve in this repository, and it MUST sit in the page's References section rather than in a heading, the table of contents, or ahead of the page's substantive content. Citing the path of any OTHER internal file, or of an evidence dossier, remains forbidden in rendered content.
